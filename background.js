...
function getMessageId(msgElement) {
    // 1. Try data-item-id first (original approach)
    const itemId = msgElement.getAttribute('data-item-id');
    if (itemId) return itemId;

    // 2. Try to extract conversation ID from URL (for group chats)
    const conversationLink = msgElement.querySelector('a[href*="/messages/"]');
    if (conversationLink) {
        const href = conversationLink.getAttribute('href');
        const matches = href.match(/\/messages\/(\d+)/);
        if (matches && matches[1]) {
            return "conv_" + matches[1];
        }
    }

    // 3. For individual messages, use the username/handle
    const userHandle = msgElement.querySelector('[dir="ltr"] span:not([dir="auto"])');
    if (userHandle && userHandle.textContent.includes('@')) {
        return "user_" + userHandle.textContent.trim();
    }

    // 4. Use profile link as fallback
    const profileLink = msgElement.querySelector('a[href^="/"]');
    if (profileLink) {
        const href = profileLink.getAttribute('href');
        if (href && href.startsWith('/') && !href.includes('/messages/')) {
            return "link_" + href.replace(/^\//, '');
        }
    }

    // 5. Last resort: Clean message content removing all timestamps
    const msgContent = msgElement.innerText || '';
    let cleanContent = msgContent
        .replace(/·\s*\d+[hmds]/g, '')  // Remove "· 7m" pattern
        .replace(/\b\d+[hmds]\b/g, '')   // Remove standalone "7m" pattern
        .replace(/·\s*(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\s+\d{1,2}/g, '') // Remove date formats
        .trim();
    
    return "txt_" + cleanContent.slice(0, 30);
}
